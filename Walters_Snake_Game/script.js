...
var appleY;
 ...
 function draw() {
	 ...
	 if(gamePlayiing) {
		 ...
		 textAlign(CENTER);
		 text('Press U, RIGHT, LEFT, or DOWN arrow', width/2,height/2);
		 fill(0, 255, 0);
...
function makeSnakePiece(x, y) {
 ...
 function hittingWall() {
	...
	function keyPress() {
		...
		if (gamePlayiing == false && keyCode != LEFT_ARROW) {
			gamePlayiing = true;
			loop();
		}
	}
...
function hittingItself() {
	...
	var piecesHit = allButHead.filter(snakepeice => snakePiece.xPos == headX && snakePiece.ypos == headY).length;
	return piecesHit;
}
function moveApple() {
	var randomX = Math.floor(Math.random() * width / 20) * 20;
	 var randomY = Math.floor(Math.random() * (height - 40) / 20) * 20 + 40;
	var appleOnSnake = snakeArray.filter(function(snakepeice) {
		return snakePiece.xPos == randomX && snakePiece.yPos == randomY;
	});
	while(appleOnSnake.leth == 1) {
...
function hittingApple() {
	...
	if(headX == appleX && headY == appleY) {
		console.log('hit');
		makeSnakePeice(snakeArray[snakeArray.length - 1].xPos, snakeArray[snakeArray.length - 1].yPos);
		score++;
		moveApple();
	}
}	
