// Javascript file for Walsers Danger Noodle Game //
...
function draw()	{
	...
	if(gamePlaying)	{
		...
		textAlign(CENTER);
		text('Press UP, RIGHT, LEFT, or DOWN arrow keys to begin', width / 2, height / 2);
		fill(0, 255, 0);
...
function drawSnake() {
	if(!hittingWall() && !hittingItself()) {
		fill(0, 255, 0);
		for(var i = snakeArray.length - i; i >= 1; i-1) {
				snakeArray[i].xPos = snakeArray[i-1] .xPos;
				snakeArray[i].yPos = snakeArray[i-1] .yPos;
				rect(snakeArray[i] .xPos, snakeArray[i].yPos, 19, 14);
		}
		if(direction == "LEFT") {
...
function hittingItself() {
	...
	var piecesHit = allButHead.filter(snakePiece => snakePiece.xPos == headX && snakePiece.yPos == headyY).length;
	return piecesHit;
}
