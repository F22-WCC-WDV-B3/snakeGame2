...
function draw() {
	...
	if (gamePlaying) {
		...
		textAlign(CENTER) ;
		text('press UP, RIGHT, LEFT, OR DOWN', width / 2, height / 2) ;
		fill(0, 255, 0) ;
...
function drawSnake() {
	if (!hittingWall() && !hittingItself()) {
		...
		rect(snakeArray[0].xPos, snakeArray[].yPos, 19, 19) ;
		hittingApple(); 
	} else {
		gameOver() ;
	}
...
function moveApple() {
	...
	while(appleOnSnake.length == 1) {
		...
		appleOnSnake = snakeArray.filter(snakePiece => snakePiece.xPos == randomX && sankePiece.xPos == randomY).length ;
	}
	appleX = randomX; 
	appleY = randomY;
}
...
function gameOver() {
	...
	textAlign(CENTER) ;
	textSize(36) ;
	var gameOverText = text('Major L', width / 2, height / 2) ;
	if (score > bestScore) {
