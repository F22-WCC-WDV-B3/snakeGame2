// Javascript file for Blal Snakegame //
var snakeArray; // var is a variable & array is a list of values 
var score;
var bestScore;
var gamePlaying;
var appleX;
...
function setup() {
	...
	frameRate(5);
	score = 0;
	bestScore = loadBestScore();// bestScore spelling
	
	snakeArray = [];
	makeSnakePiece(60,200); // 0
	makeSnakePiece(40,200); // 1
	makeSnakePiece(20,200); // 2
...
function makeSnakePiece(x, y) {
	...
	append(snakeArray, snakePiece);// snakeArray spelling
}
...
function drawSnake() {
	if(!hittingWall() && !hittingItself()) {
		...
		} else if(direction == "UP") {// no tab needed before else
...
function hittingItself() {
	...
	var allButHead = snakeArray.slice(1);// slice not slide
	var piecesHit = allButHead.filter(snakePiece => snakePiece.xPos == headX && snakePiece.yPos == headY).length;// headX
	return piecesHit;
}
